...
func InitMember(r *gin.RouterGroup) {
	r.POST("/joingroup", anthHanleCreateMember)
}

func anthHanleCreateMember(c *gin.Context) {
	openId := uitls.GetLoginOpenId(c)
	if openId == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"code": http.StatusUnauthorized})
		return
	}
	var member model.RequestMember
	...
	ok, status := dao.CreateMemer(member.Name, member.GroupId, openId)
	if ok == false {
